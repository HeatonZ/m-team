...
│   ├── schema/
│   │   ├── task.js           # 任务格式定义 + 验证工具
│   │   └── task.test.js       # Vitest 单元测试（26 个用例）
│   └── queue/
│       └── index.js          # 任务池核心操作
├── skills/
...
```json
{
  ...
  "context": [
    ...
    { "executor": "agent_2", "step": "联系供应商确认价格", "output": { "summary": "联系了5家，3家回复", "files": ["data/contact_log.md"] }, "completedAt": 1745622000000 }
  ],
  "priority": "high",
  "publisher": "user",
  "status": "pending",
...
```
26 个单元测试，覆盖 src/schema/task.js 纯函数：
- createTask / validateTask（包含 context 格式）
