...
// 返回: { claimed: true, taskId: "...", runId: "...", sessionKey: "mteam:{taskId}:{agentId}:{timestamp}" }
// 原子操作：锁文件 + 状态校验，确保只有一个 agent 能抢到
// Plugin 内部通过 api.runtime.subagent.run() 直接创建 executor session
// sessionKey 格式: mteam:{taskId}:{agentId}:{timestamp}（含时间戳，relay 后不冲突）
...
mteam_claim_task()
  ...
  └─ api.runtime.subagent.run()
       sessionKey="mteam:task123:{agentId}:{timestamp}"
