...
> 当前测试用例文档**仍有一部分沿用历史状态机口径**。请不要把本目录直接当成“已完全对齐当前代码”的事实来源。
...
- 多数 `TC-*.md` 仍保留较重的历史状态机叙述，部分内容与当前实现关注点并不完全一致
...
| 模块编号 | 文件 | 新名称 | 备注 |
|------|------|------|------|
| M1 | [TC-A.md](./TC-A.md) | 链式正常流转 | 仍偏历史口径，需要后续收敛 |
| M2 | [TC-B.md](./TC-B.md) | 交接与返工 | 需复核是否与当前 relay / reject 行为一致 |
...
| M9 | [TC-I.md](./TC-I.md) | 持久化一致性 | 需对照 `task/context` 当前落盘结构 |
| M10 | [TC-J.md](./TC-J.md) | 排队与优先级 | 仍有旧待认领阶段叙述 |
| M11 | [TC-K.md](./TC-K.md) | 底层存储映射 | 需复核 |
| M12 | [TC-L.md](./TC-L.md) | 查询与看板数据 | 仍引用旧细粒度阶段细节，需要收敛 |
...
2. **再改 TC-A / TC-B**：把 full-chain relay / reject / close 主路径对齐当前代码
3. **再改 TC-L / TC-J**：清理旧查询口径和旧待认领阶段叙述
4. **最后统一过一轮术语**：
   - `status`
   - `agent_end` 裁决
