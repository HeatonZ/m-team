...
1. mteam_get_agent_active({ agentId })
   └── 有 running 任务 → mteam_update_task({ lastHeartbeatAt }) → 结束
2. 无 running 任务 → mteam_get_pending({ agentId })
   └── 有待认领 → mteam_claim_task → 获得 taskId
3. 继续等待下一轮心跳
```
...
api.on('subagent_ended', async (event) => {
  ...
  const taskId = targetSessionKey.split(':')[1];
...
T0                mteam_claim_task()
                    ...
                    └─ api.runtime.subagent.run()
                         sessionKey="mteam:task123:agent1:1745740800000"
                         message="[M-Team Task...]"
...
mteam_update_task({
  ...
  → status=pending                   mteam_claim_task()
  → executor=null                      → status=running
  → lastExecutor=agent1               → executor=agent2
  → description="下一步描述"
  session 结束
...
超过 30 分钟 `lastHeartbeatAt` 未更新的任务视为疑似僵尸，由下一轮 heartbeat 自动处理（relinquish 或忽略）。
