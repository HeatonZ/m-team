...
**Description 填写规范：**
- 只写当前 executor 要做的这一件事，**不写输出格式和路径**
- 格式：`动词 + 关键词 + 筛选条件`
...
| Wrong | Correct |
|-------|---------|
| goal = "Agent 自我介绍任务：体现 Manager 风格" | goal = "在飞书单聊（chat_id: xxx）中发送一条300字以内的自我介绍" |
| goal = "收到任务的 agent 完成自我介绍" | goal = 具体可验证的结果描述 |
| description = "在飞书发一条自我介绍"（无上下文） | description = 具体的工具调用指令（to=chat_id, message=内容） |
| description = goal | description 写第一步，goal 写任务终点 |
| description = "输出到 /tmp/result.json" | description = "抓取页面数据"（executor 自己决定输出路径） |
...
| goal = "帮我调研供应商" | goal = "从1688筛选3家供应商，输出到 xxx" |
