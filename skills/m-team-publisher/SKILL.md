...
| 8 | description 不写发送渠道 | description 中不出现"发到飞书"、"发Discord"、"发送到群"等渠道关键词；若真要外发，必须把外发动作写成明确业务动作而非渠道口号？ |
...
| 12 | 渠道/人设没有冒充业务动作 | description 没有把"体现 Manager 风格"、"发给群里"、"让对方知道"这类表达冒充成业务动作？ |
...
| description = "在飞书发一条自我介绍"（无上下文） | #2, #5, #8, #12 |
| description = goal（两者完全相同） | #4 |
| description = "回复收到" | #9 |
| 任务一步就能完成但发到了池子 | #6 |
| description = "先查再筛再汇总发群里" | #11, #12 |
