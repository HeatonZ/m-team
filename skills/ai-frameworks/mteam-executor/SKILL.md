...
Executor 拿到任务后的完整决策框架——如何判断接单、如何执行单步、如何判断交接、如何写 context output、何时升级。
...
```
contextStep.step = "交接给 {nextRole}：{具体下一步做什么}"
contextStep.output = {
  "summary": "{这步完成的内容摘要}",
  "relay_to": "{nextRole}",
...
| complete 后 Publisher 说不合格 | output.summary 写了"已完成"但没有数据 | summary 必须包含具体结果（数据/文件/结论） |
...
```
□ 这一步的完成标准我是否清楚？
□ output.summary 是否包含具体结果（不是"进行中"或"已完成"）？
...
- **IDENTITY.md**（每个角色的身份定义）位于 `{repo}/executors/{maker,fixer,scholar,captain}/IDENTITY.md`
- **SOUL.md**（每个角色的决策原则）位于 `{repo}/executors/{maker,fixer,scholar,captain}/SOUL.md`
- **AGENTS.md**（每个角色的运行规范）位于 `{repo}/executors/{maker,fixer,scholar,captain}/AGENTS.md`
